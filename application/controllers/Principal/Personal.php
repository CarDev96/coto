...
class Personal extends CI_Controller {
	...
	public function index()
	{
        ...
        $data = array(
            ...
            'info_personal' => $this->Personal_model->getinfo(),
    ...
    public function nuevo()
    {
        ...
        $nombre_g = $this->input->post("nombre_g"); //id_casa

        $telefono_g = $this->input->post("telefono_g"); //id_concepto_in

        $turno_g = $this->input->post("turno_g"); //ingreso

        $email_g = $this->input->post("email_g");

        $domicilio_g = $this->input->post("domicilio_g");

		$dd = explode("(",$telefono_g);
    
		$dd2 = explode(")",$dd[1]);    
			
		$dd3 = explode("-",$dd2[1]);   
		
		$telefono_g2 = $dd[0].$dd2[0].$dd3[0].$dd3[1];        
 
        $data = array(

            'nombre_g' => $nombre_g,

            'telefono_g' => $telefono_g2,

            'turno_g' => $turno_g,

            'email_g' => $email_g,

            'domicilio_g' => $domicilio_g,
        ...
        if ($this->Personal_model->save($data)) {
            ...
            redirect(base_url() . "Principal/Personal/add");
        } else {
